...
class Alu_api_t: public sim_api_t<VerilatorDataWrapper*> {
    ...
    void init_sim_data() {
        ...
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_ALU_Sel)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_B)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_A)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_CarryOut)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_ALU_Out)));
...
int main(int argc, char **argv, char **env) {
    ...
    VAlu* top = new VAlu;
    std::string vcdfile = "test_run_dir/ALU__Arithmetic_Logical_Unit/Alu.vcd";
    ...
    Verilated::mkdir("test_run_dir/ALU__Arithmetic_Logical_Unit/logs");
    VerilatedCov::write("test_run_dir/ALU__Arithmetic_Logical_Unit/logs/coverage.dat");
#endif
    delete top;
    exit(0);
}
