...
#endif
#include <iostream>
class MDU_api_t: public sim_api_t<VerilatorDataWrapper*> {
    public:
    MDU_api_t(VMDU* _dut) {
        dut = _dut;
        main_time = 0L;
    ...
    void init_sim_data() {
        ...
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_i_mdu_valid)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_i_mdu_op)));
        sim_data.inputs.push_back(new VerilatorIData(&(dut->io_i_mdu_rs2)));
        sim_data.inputs.push_back(new VerilatorIData(&(dut->io_i_mdu_rs1)));
        sim_data.outputs.push_back(new VerilatorIData(&(dut->io_o_mdu_rd)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_o_mdu_ready)));
        sim_data.signals.push_back(new VerilatorCData(&(dut->reset)));
        sim_data.signal_map["MDU.reset"] = 0;
    }
#if VM_TRACE
...
int main(int argc, char **argv, char **env) {
    ...
    VMDU* top = new VMDU;
    std::string vcdfile = "test_run_dir/MULTIPLICATION_AND_DIVISION_UNIT/MDU.vcd";
    ...
    Verilated::mkdir("test_run_dir/MULTIPLICATION_AND_DIVISION_UNIT/logs");
    VerilatedCov::write("test_run_dir/MULTIPLICATION_AND_DIVISION_UNIT/logs/coverage.dat");
#endif
    delete top;
    exit(0);
}
