...
#endif
#include <iostream>
class Mux2_api_t: public sim_api_t<VerilatorDataWrapper*> {
    public:
    Mux2_api_t(VMux2* _dut) {
        dut = _dut;
        main_time = 0L;
        is_exit = false;
#if VM_TRACE
    ...
    void init_sim_data() {
        ...
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_S)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_D1)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_D0)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_Y)));
        sim_data.signals.push_back(new VerilatorCData(&(dut->reset)));
        sim_data.signal_map["Mux2.reset"] = 0;
    }
#if VM_TRACE
...
int main(int argc, char **argv, char **env) {
    ...
    VMux2* top = new VMux2;
    std::string vcdfile = "test_run_dir/Multiplexer_2BIT/Mux2.vcd";
    ...
    Verilated::mkdir("test_run_dir/Multiplexer_2BIT/logs");
    VerilatedCov::write("test_run_dir/Multiplexer_2BIT/logs/coverage.dat");
#endif
    delete top;
    exit(0);
}
